...
class TestTokenFamilyModels(TestCase):
    ...
    def test_family_id_cannot_be_null(self):
        ...
        ):  # Catch either potential exception
    ...
    def test_user_can_have_multiple_families(self):
        ...
        self.assertEqual(
            TokenFamily.objects.filter(user=self.user).count(), num_families_to_create
        )
...
class TestTokenFamilyInRefreshTokens(TestCase):
    ...
    def test_families_are_added_to_the_family_list(self):
        ...
        self.assertEqual(
            token_family_obj.expires_at.replace(microsecond=0),
            token.get_family_expiration_date().replace(microsecond=0),
        )
    ...
    def test_family_attributes_do_not_change_for_new_tokens(self):
        ...
        self.assertEqual(
            token_1[api_settings.TOKEN_FAMILY_CLAIM],
            token_2[api_settings.TOKEN_FAMILY_CLAIM],
        )
        self.assertEqual(
            token_1[api_settings.TOKEN_FAMILY_EXPIRATION_CLAIM],
            token_2[api_settings.TOKEN_FAMILY_EXPIRATION_CLAIM],
        )
    ...
    def test_token_family_with_modify_family_expiration_claim(self):
        ...
        self.assertEqual(
            api_settings.TOKEN_FAMILY_EXPIRATION_CLAIM, "fam_test_exp_claim"
        )
    ...
    def test_user_can_have_multiple_families(self):
        amount_of_tokens = 5
        tokens = [RefreshToken.for_user(self.user) for _ in range(amount_of_tokens)]
...
class TestTokenFamilyInAccessTokens(TestCase):
    ...
    @override_api_settings(
        TOKEN_FAMILY_CHECK_ON_ACCESS=True, TOKEN_FAMILY_LIFETIME=timedelta(minutes=30)
    )
...
class TestFlushExpiredTokenFamiliesCommand(TestCase):
    ...
    def test_it_should_delete_any_expire_families(self):
        ...
        valid_family_tokens: list[RefreshToken] = [
            RefreshToken.for_user(self.user) for _ in range(amount_of_valid_families)
        ]
        ...
        self.assertEqual(
            ...
            amount_expired_blacklisted_families
            + amount_not_expired_blacklisted_families,
        )

        call_command("flushexpiredfamilies")

        self.assertEqual(TokenFamily.objects.all().count(), amount_of_valid_families)
        self.assertEqual(
            BlacklistedTokenFamily.objects.all().count(),
            amount_not_expired_blacklisted_families,
        )
